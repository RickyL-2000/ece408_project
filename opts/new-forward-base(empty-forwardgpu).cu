...
#define TEST_NAME "baseline 10000 empty conv_forward_gpu"

#define BLOCK_WIDTH 16
// #define CONV_DEBUG

/*
 * NOTE: Apparently, empty the logprog function can significantly "increase" the performance
 * */
...
#define k4d(i3, i2, i1, i0) k[(i3) * (C * K * K) + (i2) * (K * K) + (i1) * (K) + i0]
    ...
    int h = (blockIdx.z / W_num) * BLOCK_WIDTH + threadIdx.y,   // FIXME: maybe wrong
    ...
    const int B, const int M, const int C, const int H, const int W, const int K)
{
    ...
    conv_forward_kernel<<<dimGrid, dimBlock>>>(*device_y_ptr, *device_x_ptr, *device_k_ptr, B, M, C, H, W, K);
    ...
    // int y_size = B*M*H_out*W_out*sizeof(float);
        // x_size = B*C*H*W*sizeof(float), 
        // k_size = M*C*K*K*sizeof(float);
    cudaMemcpy(host_y, device_y_ptr, y_size, cudaMemcpyDeviceToHost);
    // Free device memory
